...
const GirisSlide = ({ setActiveSlideId }) => {
    ...
    const getCategory = (type) => {
        if (['usul_esas', '2025_yuklenici_gerceklesme', 'yer_teslimi', 'yuklenici_bilgileri'].includes(type)) return 'strateji';
        if (['chart_pie', 'chart_s1_s2', 's1_s2_denetleme', 'afet', 'cbs_durumu'].includes(type)) return 'analiz';
        if (['photo_ag', 'photo_yg'].includes(type)) return 'saha';
        if (['kesif_ozeti'].includes(type)) return 'planlama';
        return 'diger';
    };

    const categories = {
        strateji: { title: "Strateji ve Hedefler", color: "text-blue-500", bg: "bg-blue-500/10", border: 'border-blue-500/20', icon: <Briefcase size={20} /> },
        analiz: { title: "Analiz, Denetim ve CBS", color: "text-rose-500", bg: "bg-rose-500/10", border: 'border-rose-500/20', icon: <Activity size={20} /> },
        saha: { title: "Saha Verileri", color: "text-purple-500", bg: "bg-purple-500/10", border: 'border-purple-500/20', icon: <ImageIcon size={20} /> },
        planlama: { title: "Gelecek Planlaması", color: "text-emerald-500", bg: "bg-emerald-500/10", border: 'border-emerald-500/20', icon: <Map size={20} /> },
        diger: { title: "Diğer İncelemeler", color: "text-gray-500", bg: "bg-gray-500/10", border: 'border-gray-500/20', icon: <Bookmark size={20} /> }
    };
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 flex flex-col items-center justify-center w-full max-w-[95rem] mx-auto px-6 lg:px-12 py-4 z-10 space-y-12">
                ...
                <section className={`w-full transition-all duration-1000 delay-300 ease-out ${isMounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 xl:gap-8 items-start">
                        {['strateji', 'analiz', 'saha', 'planlama', 'diger'].map((key) => {
                            ...
                            return (
                                <div key={key} className="flex flex-col gap-4">
                                    <h3 className={`text-base lg:text-lg font-extrabold flex items-center gap-2 border-b pb-2 ${catMetaData.border}`}>
