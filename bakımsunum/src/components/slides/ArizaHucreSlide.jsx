...
import { Layout, Table2, BarChart3, TrendingUp, MapPin, Search, X, Maximize, Minimize, ArrowUp, ArrowDown, Activity } from 'lucide-react';
import { arizaHucreBakim25 } from '../../data/mockData';
...
export default function ArizaHucreSlide() {
    const [viewMode, setViewMode] = useState('chart'); // 'chart' or 'table'
    const [searchTerm, setSearchTerm] = useState('');
    const [selectedBm, setSelectedBm] = useState('TOROSLAR');
    const [isFullscreen, setIsFullscreen] = useState(() => !!document.fullscreenElement);
    const [sortConfig, setSortConfig] = useState({ key: 'toplam', direction: 'desc' });
    ...
    const rawData = arizaHucreBakim25;
    const uniqueBms = useMemo(() => ['TOROSLAR', ...new Set(rawData.map(d => d.bolge))].sort(), [rawData]);

    const filteredData = useMemo(() => {
        let data = rawData.filter(d => {
            const matchesBm = selectedBm === 'TOROSLAR' || d.bolge === selectedBm;
            const matchesSearch = d.om.toLowerCase().includes(searchTerm.toLowerCase()) || d.bolge.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    const totals = useMemo(() => {
        const totalCount = filteredData.reduce((acc, curr) => acc + curr.toplam, 0);
        const omCount = filteredData.length;
        const avgCount = omCount > 0 ? (totalCount / omCount).toFixed(2) : 0;
        return { totalCount, omCount, avgCount };
    }, [filteredData]);
    ...
    const chartOptions = useMemo(() => ({
        ...
        colors: filteredData.map(d => BM_COLORS[d.bolge]?.border || '#94a3b8'),
        dataLabels: {
            enabled: true,
            formatter: (val) => val,
            offsetY: -20,
        ...
        yaxis: {
            title: { text: 'Toplam Hücre Sayısı', style: { fontWeight: 700 } },
    ...
    const chartSeries = [{
        name: 'Hücre Bakımı',
        data: filteredData.map(d => d.toplam)
    }];
        ...
        <div className={`flex flex-col h-full w-full bg-base-100 ${isFullscreen ? 'fixed inset-0 z-50 p-8' : 'p-4 md:p-6'} overflow-hidden`}>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <h2 className={`font-black tracking-tight ${isFullscreen ? 'text-3xl' : 'text-2xl'}`}>2025 ARIZA KAPSAMINDA KULLANILAN 1-KODLU  HÜCRELER</h2>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 shrink-0">
                <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-2xl flex items-center justify-between shadow-sm">
                    <div>
                        <p className="text-emerald-600 text-[10px] font-black uppercase tracking-widest mb-1">TOPLAM HÜCRE SAYISI</p>
                        <p className={`${isFullscreen ? 'text-3xl' : 'text-2xl'} font-black text-emerald-950`}>{totals.totalCount} <span className="text-sm font-bold">Adet</span></p>
                    </div>
                ...
                <div className="bg-amber-50 border border-amber-100 p-4 rounded-2xl flex items-center justify-between shadow-sm">
                    <div>
                        ...
                        <p className={`${isFullscreen ? 'text-3xl' : 'text-2xl'} font-black text-amber-950`}>{totals.avgCount} <span className="text-sm font-bold">Adet</span></p>
                    </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4 bg-base-200/50 p-3 rounded-2xl border border-base-200 shrink-0">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <ExportExcelButton data={filteredData} fileName={`Ariza_Hucre_2025`} />
                </div>
            </div>
                        ...
                        <table className="table table-pin-rows table-md">
                            <thead className="sticky top-0 bg-base-200 z-10">
                                <tr className="text-xs uppercase font-black opacity-60">
                                    <th className="cursor-pointer hover:text-primary transition-colors" onClick={() => handleSort('bolge')}>
                                        <div className="flex items-center gap-2">
                                            Bölge {sortConfig.key === 'bolge' && (sortConfig.direction === 'asc' ? <ArrowUp size={12} /> : <ArrowDown size={12} />)}
                                        </div>
                                    </th>
                                    ...
                                    <th className="text-right cursor-pointer hover:text-primary transition-colors" onClick={() => handleSort('toplam')}>
                                        <div className="flex items-center justify-end gap-2">
                                            Toplam Hücre {sortConfig.key === 'toplam' && (sortConfig.direction === 'asc' ? <ArrowUp size={12} /> : <ArrowDown size={12} />)}
                                        </div>
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr key={`${row.om}-${idx}`} className="hover:bg-base-200/40 transition-colors">
                                        <td>
                                            <span
                                                ...
                                                style={{ backgroundColor: BM_COLORS[row.bolge]?.bg, color: BM_COLORS[row.bolge]?.text }}
                                            >
                                                {row.bolge}
                                            </span>
                                        </td>
                                ...
                                {filteredData.length > 0 && (
                                    <tr className="bg-primary/5 font-black text-primary border-t-2 border-primary/20">
                                        ...
                                        <td className="text-right font-mono text-lg">{totals.totalCount} Adet</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
